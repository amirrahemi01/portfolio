...
function Home() {
  ...
  // If navigated with hash (e.g., from project detail -> /#projects), scroll to section
  useEffect(() => {
    if (location.hash) {
      ...
      <main className="flex flex-col justify-center w-full lg:w-3/4 my-0 mx-auto">
        ...
        <section className="relative h-screen sm:h-[96vh] w-full max-w-none flex items-center justify-center flex-col bg-black dark:bg-white rounded-none sm:rounded-2xl overflow-hidden">
          <div className="absolute top-20 md:top-40 lg:top-8 px-4 py-3 w-full sm:w-auto text-center sm:text-left">
            <h2 className={clsx("text-7xl sm:text-3xl md:text-5xl lg:text-8xl text-white dark:text-black leading-tight", currentLang === "fa" ? "text-right" : "text-left")}>
              {t("header.hello")}
            </h2>
  ...
  return (
    <>
      <Routes>
        ...
        <Route path="/fa/projects/:slug" element={<ProjectDetail />} />
        <Route path="/project/:slug" element={<ProjectDetail />} />
        <Route path="/fa/project/:slug" element={<ProjectDetail />} />
      </Routes>
      <SpeedInsights />
    </>
  );
