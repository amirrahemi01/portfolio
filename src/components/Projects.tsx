  ...
  return (
    <div className="w-full py-12 md:py-20 px-4" id="projects">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold mb-4 text-center pb-4 md:pb-8 px-4 bg-gradient-to-r from-black to-gray-500 bg-clip-text text-transparent dark:from-white dark:to-gray-400">
          {t("content.projects")}
        </h1>
        ...
        >
          {projects.map((project) => (
            <div
              key={project.id}
              className={`group relative rounded-2xl md:rounded-3xl overflow-hidden backdrop-blur-xl bg-gradient-to-br ${
              ...
              style={{
                animation: `fadeInUp 0.6s ease-out ${project.delay} both`,
              }}
            >
              <div
                className={`relative overflow-hidden flex-shrink-0 cursor-pointer h-48 md:h-64 ${
                  viewMode === "list" ? "lg:w-80" : ""
                }`}
              >
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${project.overlayFrom} ${project.overlayTo} z-10`}
                ></div>
                <img
                  src={project.image}
                  alt={project.title}
              ...
              <div
                className={`relative p-6 md:p-8 flex flex-col flex-grow cursor-pointer ${
                  viewMode === "list" ? "lg:justify-center" : ""
                }`}
              >
                <div className="flex-grow">
                  <h2
                    className={`text-2xl sm:text-3xl md:text-4xl font-bold text-black dark:text-white mb-3 md:mb-4 ${project.hoverColor} group-hover:bg-clip-text group-hover:bg-gradient-to-r ${project.hoverTextFrom} ${project.hoverTextTo} transition-all duration-300`}
                  >
                    {project.title}
                  </h2>
                  <p className="text-black dark:text-slate-300 text-sm md:text-base leading-relaxed mb-4 md:mb-6">
                    {project.description}
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-auto">
                  {project.tags.map((tag, index) => (
                    <p
                      key={index}
                      className="px-3 md:px-4 py-1.5 md:py-2 rounded-full bg-black/5 dark:bg-white/5 backdrop-blur-sm border border-black/10 dark:border-white/10 text-xs md:text-sm text-gray-100 dark:text-slate-300 hover:bg-white/10 hover:border-white/20 transition-all duration-300"
                    >
                      {tag}
                    </p>
                  ))}
                ...
                <div
                  className={`absolute -bottom-32 -right-32 w-64 h-64 bg-gradient-to-r ${project.hoverGlow} rounded-full blur-3xl opacity-0 group-hover:opacity-30 transition-opacity duration-700 pointer-events-none`}
                ></div>
              </div>
              ...
              <div
                className={`absolute -top-3 -right-3 md:-top-4 md:-right-4 w-10 h-10 md:w-12 md:h-12 rounded-full bg-gradient-to-r ${project.badgeGradient} flex items-center justify-center text-white font-bold text-sm md:text-base shadow-lg group-hover:scale-125 group-hover:rotate-12 transition-all duration-500`}
              >
                {project.id}
              </div>
            </div>
